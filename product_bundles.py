...
def get_discounted_bundles(api_name, segment_names):
    ...
    for segment_name in segment_names:
        bundles_content = multipart_download_from_s3(S3_BUCKET_NAME, f"{api_name}/{segment_name}_itemsets.csv")
        products_content = multipart_download_from_s3(S3_BUCKET_NAME, f"{api_name}/products.csv")

        if bundles_content is None or products_content is None:
            continue

        bundles_df = pd.read_csv(StringIO(bundles_content))
        products_df = pd.read_csv(StringIO(products_content))
        stock_code_to_description = dict(zip(products_df['StockCode'], products_df['Description']))

        bundle_info = []
        for bundle_str in bundles_df['itemset'].head(10):
            bundle_stock_codes = [code.strip().strip("'") for code in bundle_str.strip('{}').split(',')]
            bundle_description = []
            actual_price = 0
            discounted_price = 0
        ...
        bundle_info_df = pd.DataFrame(bundle_info)
        bundle_info_content = bundle_info_df.to_csv(index=False)
        multipart_upload_to_s3(bundle_info_content, S3_BUCKET_NAME, f"{api_name}/{segment_name}_discounted_bundles.csv")
        bundle_info_dict[segment_name] = bundle_info_df
...
def get_association_info(api_name, segment_names):
    ...
    products_content = multipart_download_from_s3(S3_BUCKET_NAME, f'{api_name}/products_data.csv')
    if products_content is None:
        return None
    ...
    for segment_name in segment_names:
        associations_content = multipart_download_from_s3(S3_BUCKET_NAME, f"{api_name}/{segment_name}_associations.csv")
        if associations_content is None:
            continue

        associations_df = pd.read_csv(StringIO(associations_content))
        association_info = []
    ...
    all_associations_df = pd.DataFrame(all_associations)
    all_associations_content = all_associations_df.to_csv(index=False)
    multipart_upload_to_s3(all_associations_content, S3_BUCKET_NAME, f"{api_name}/bogd_offers.csv")
...
def save_custom_discounts(data, file_name, root_path):
    ...
    multipart_upload_to_s3(file_content, S3_BUCKET_NAME, f"{root_path}{file_name}")
...
def get_bundlesets(high_bundle, risk_bundle, nurture_bundle, root_path):
    ...
    for bundle_name, bundle_df in bundle_data.items():
        ...
        associations_content = associations.to_csv(index=False)
        itemsets_content = itemsets.to_csv(index=False)

        multipart_upload_to_s3(associations_content, S3_BUCKET_NAME, f"{root_path}{bundle_name}_associations.csv")
        multipart_upload_to_s3(itemsets_content, S3_BUCKET_NAME, f"{root_path}{bundle_name}_itemsets.csv")

        print(f"Bundle '{bundle_name}' associations and itemsets saved to S3.")
...
def create_bogd_offers(root_path, segment_names=['high_value', 'nurture', 'risk']):
    association_info_dict = {}
    all_associations = []

    products_content = multipart_download_from_s3(S3_BUCKET_NAME, f'{root_path}/products_data.csv')
    if products_content is None:
        return None
    ...
    for segment_name in segment_names:
        associations_content = multipart_download_from_s3(S3_BUCKET_NAME, f"{root_path}/{segment_name}_associations.csv")
        if associations_content is None:
            continue
    ...
    all_associations_df = pd.DataFrame(all_associations)
    all_associations_content = all_associations_df.to_csv(index=False)
    multipart_upload_to_s3(all_associations_content, S3_BUCKET_NAME, f"{root_path}/bogd_offers.csv")
