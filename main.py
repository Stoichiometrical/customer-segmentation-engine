...
config = boto3.client('s3', config=Config(
    retries={
        'max_attempts': 10,
        'mode': 'standard'
    },
...
def multipart_upload_to_s3(file_content, bucket_name, s3_key):
    try:
        transfer_config = boto3.s3.transfer.TransferConfig(multipart_threshold=1024*25, multipart_chunksize=1024*25)
        print(f"Uploading to S3: {s3_key}")
        s3_client.upload_fileobj(BytesIO(file_content.encode('utf-8')), bucket_name, s3_key, Config=transfer_config)
...
def multipart_download_from_s3(bucket_name, s3_key):
    try:
...
def initiate_segments():
    try:
        ...
        sales_file_content = multipart_download_from_s3(S3_BUCKET_NAME, sales_file_path)
        customer_file_content = multipart_download_from_s3(S3_BUCKET_NAME, customer_file_path)
        print("Downloaded files from S3")

        if sales_file_content is None or customer_file_content is None:
        ...
        products_data = generate_products_data(sales_df)
        products_data_csv = products_data.to_csv(index=False)
        products_data_path = f"{folder_name}/products_data.csv"
        print(f"Uploading products data to S3: {products_data_path}")
        if not multipart_upload_to_s3(products_data_csv, S3_BUCKET_NAME, products_data_path):
            raise Exception("Failed to upload products data to S3.")
        print("Products data uploaded to S3")
        ...
        multipart_upload_to_s3(summary_csv, S3_BUCKET_NAME, f"{folder_name}/segment_data.csv")
        multipart_upload_to_s3(merged_csv, S3_BUCKET_NAME, f"{folder_name}/segment_transactions.csv")
        multipart_upload_to_s3(segment_stats_csv, S3_BUCKET_NAME, f"{folder_name}/segment_stats.csv")
        multipart_upload_to_s3(subsegment_stats_csv, S3_BUCKET_NAME, f"{folder_name}/subsegment_stats.csv")
        multipart_upload_to_s3(percentages_json, S3_BUCKET_NAME, f"{folder_name}/segment_percentages.json")
        multipart_upload_to_s3(all_data_json, S3_BUCKET_NAME, f"{folder_name}/segment_compositions.json")
        ...
        high_value_path = f"{folder_name}/high_value_transactions.csv"
        risk_path = f"{folder_name}/risk_transactions.csv"
        nurture_path = f"{folder_name}/nurture_transactions.csv"
        multipart_upload_to_s3(high_value_df.to_csv(index=False), S3_BUCKET_NAME, high_value_path)
        multipart_upload_to_s3(risk_df.to_csv(index=False), S3_BUCKET_NAME, risk_path)
        multipart_upload_to_s3(nurture_df.to_csv(index=False), S3_BUCKET_NAME, nurture_path)
        print("Segment transactions uploaded to S3")
...
def get_customer_segments():
    ...
    customer_segments_content = multipart_download_from_s3(S3_BUCKET_NAME, customer_segments_path)
...
def get_percentages(api_name):
    ...
    percentages_content = multipart_download_from_s3(S3_BUCKET_NAME, file_path)
...
def generate_api_data(api_name, project_name):
    try:
        ...
        high_content = multipart_download_from_s3(S3_BUCKET_NAME, high_path)
        nurture_content = multipart_download_from_s3(S3_BUCKET_NAME, nurture_path)
        risk_content = multipart_download_from_s3(S3_BUCKET_NAME, risk_path)
        ...
        high = pd.read_csv(StringIO(high_content), chunksize=10000)
        nurture = pd.read_csv(StringIO(nurture_content), chunksize=10000)
        risk = pd.read_csv(StringIO(risk_content), chunksize=10000)

        sales_data_path = f"{project_name}/segment_transactions.csv"
        segment_data_path = f"{project_name}/segment_data.csv"
        sales_data_content = multipart_download_from_s3(S3_BUCKET_NAME, sales_data_path)
        segment_data_content = multipart_download_from_s3(S3_BUCKET_NAME, segment_data_path)
        ...
        sales_data = pd.read_csv(StringIO(sales_data_content), chunksize=10000)
        segment_data = pd.read_csv(StringIO(segment_data_content), chunksize=10000)
        min_discount = float(data['marginReduction']['min'])
        max_discount = float(data['marginReduction']['max'])

        print("Creating marketing data...")
        for segment_chunk in segment_data:
            create_marketing_data(segment_chunk, report_path)

        products_data_path = f"{report_path}products_data.csv"
        print("Generating products data...")
        for sales_chunk in sales_data:
            data = generate_products_data(df=sales_chunk, min_margin=min_discount, min_discount=max_discount)
            chunk = data.to_csv(index=False)
            multipart_upload_to_s3(chunk, S3_BUCKET_NAME, products_data_path)

        print("Creating bundlesets...")
        for high_chunk, risk_chunk, nurture_chunk in zip(high, risk, nurture):
            get_bundlesets(high_chunk, risk_chunk, nurture_chunk, root_path=report_path)

        segment_names = ['high_value', 'nurture', 'risk']
        print("Creating bundle info...")
        generate_bundle_info(report_path, segment_names)
        create_bogd_offers(report_path)
...
def get_products():
    try:
        ...
        products_data_content = multipart_download_from_s3(S3_BUCKET_NAME, file_path)
...
def order_analysis_offers():
    try:
        ...
        segment_transactions_content = multipart_download_from_s3(S3_BUCKET_NAME, file_path)
        ...
        if not multipart_upload_to_s3(segment_transactions.to_csv(index=False), S3_BUCKET_NAME, segment_path):
