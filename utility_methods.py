...
def segment_customers(rfm_column):
    ...
    for rfm in rfm_column:
        recency, frequency, monetary = int(rfm[0]), int(rfm[1]), int(rfm[2])
...
def generate_products_data(df, min_margin=0.02, min_discount=0.02, max_discount=0.50, output_file="products_data.csv"):
    data = df[["id", "Description", "StockCode", "UnitPrice"]].drop_duplicates(subset=['StockCode'])

    def generate_profit_margin(unit_price):
        random_margin = np.random.uniform(min_margin, 1.0)
        profit_margin = unit_price * random_margin
        return round(profit_margin, 2)

    def calculate_discounted_price(unit_price, profit_margin):
        max_discount = 1 - profit_margin / unit_price
        discount = np.random.uniform(min_discount, max_discount)
        discounted_price = round(unit_price * (1 - round(discount, 2)), 2)
        return discounted_price, discount
    ...
    products_data_csv = data.to_csv(index=False)
    multipart_upload_to_s3(products_data_csv, S3_BUCKET_NAME, output_file)
...
def generate_bundle_info(api_name, segment_names):
    ...
    for segment_name in segment_names:
        bundles_filename = f"{api_name}/{segment_name}_itemsets.csv"
        bundles_content = multipart_download_from_s3(S3_BUCKET_NAME, bundles_filename)
        bundles_df = pd.read_csv(StringIO(bundles_content))

        first_10_bundles = bundles_df['itemset'].head(10)

        products_filename = f"{api_name}/products_data.csv"
        products_content = multipart_download_from_s3(S3_BUCKET_NAME, products_filename)
        ...
        for bundle_str in first_10_bundles:
            bundle_stock_codes = [code.strip().strip("'") for code in bundle_str.strip('{}').split(',')]
            bundle_description = []
            actual_price, discounted_price = 0, 0
            ...
            bundle_info.append({
                'id': str(uuid),
                'bundle': ', '.join(bundle_description),
                'actual_price': actual_price,
                'discounted_price': round(discounted_price, 2)
            })

        bundle_info_df = pd.DataFrame(bundle_info)
        output_filename = f"{api_name}/{segment_name}_bundles_info.csv"
        bundle_info_csv = bundle_info_df.to_csv(index=False)
        multipart_upload_to_s3(bundle_info_csv, S3_BUCKET_NAME, output_filename)
